...
2. **Pick the review mode BEFORE dispatch** — it changes how you dispatch. Ask in order, first hit wins:
   0. Would you read the whole diff anyway (small function, one-file tweak)? → **Don't dispatch.** Do it in the main thread. Delegation must be net token-SAVING for the orchestrator — on a small chore, brief + report + full-diff read costs more than writing the change yourself. A peon saves provider-side labor; it must never increase orchestrator-side reading.
   1. No runnable verify command in the repo, or style/architecture is the point of the task — and the final diff is still clearly cheaper to READ than to produce (bulk mechanical sweep, exploration-heavy chore whose dead ends burn the provider's tokens, not yours)? → **Classic.**
   ...
   - **Test-gated dispatch** (high-risk escalation of black-box: auth, billing, migrations — where even an audited peon-authored suite isn't enough): YOU author the tests in the orchestration thread, commit them, and run the verify once to prove them red on base. Then dispatch with `--verify` pointing at them and an `--allow` that EXCLUDES the test paths — any peon edit to a test file is a scope violation, i.e. the tests are locked read-only with no extra tooling. Highest orchestrator up-front cost (you write tests, not just read them) — never the default — but it is the treatment that saves the most tokens per round after that: the suite is authored once, gates every poke round for free, and the implementation is never read.
   - **Classic full-diff review** (suite-less repos or style-is-the-point tasks, and ONLY when reading the final diff beats producing it): dispatch with the criteria in the task text; you will read the whole diff. If that read wouldn't be clearly cheaper than writing the change, step 0 already kept the task in the main thread — never dispatch a peon just to re-read everything it did.
3. **Dispatch.** One peon per task. Parallel peons = *different* tasks (never the same task to two providers — that's plan-check's council, not labor). Provider: user's choice if stated, else either. Dirty repo? Ask the user before `--force` — dispatch branches from the last *commit*, so uncommitted work is invisible to the peon. While a black-box peon works, pre-author your independent probes — the wait is free. Dispatch is synchronous (minutes); if you background it through an orchestrating harness, know the harness can kill the wrapper — and the provider child dies with it (two codex dispatches died this way mid-task, live 2026-08-03). From such a harness, detach instead: `nohup <dispatch-script> &` with a `.done` marker touched on exit, and watch the marker, not the wrapper. A killed dispatch leaves a commit-less worktree — `peon scrap` and re-dispatch.
4. **Review.**
...
- **Every merge gets exactly one review treatment: the full-diff read (classic), the complete black-box gate set (check + test audit + probes; test-gated is this with foreman-authored tests), or the spot set (foreman-run verify + diffstat + load-bearing hunks). Never none.** Draft-until-reviewed is the whole point; `merge` mechanically enforces recorded scope/verify gates, and `--unchecked` is for deliberate, stated overrides only.
- **Record `--verify` at dispatch whenever the repo has a runnable suite, whatever treatment you picked** — it costs nothing, and `merge` then mechanically refuses failed, stale, or never-run verifies. A verify-less dispatch into a repo with tests needs a stated reason.
